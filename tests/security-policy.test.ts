...
describe("repository security policy", () => {
  ...
  it("checks shipped dependencies for moderate or higher advisories in local and remote gates", () => {
    const manifest = JSON.parse(readFileSync("package.json", "utf8"));
    expect(manifest.scripts["check:security"]).toBe(
      "npm audit --omit=dev --audit-level=moderate --package-lock-only --ignore-scripts",
    );
  ...
  it("makes the production-only audit scope explicit when npm is configured for production", () => {
    ...
    expect(config.omit).toEqual(["dev"]);

...
export function checkoutContext(repo, { expectedHead, expectedRoot, requireClean = false } = {}) {
  ...
  return { root, controlRoot, checkoutRole: root === controlRoot ? 'control' : 'worktree',
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  assert(args.every(arg => arg === '--require-clean' || /^--expected-head=[a-f0-9]{40}$/.test(arg)
    || /^--expected-root=.+$/.test(arg)), 'Unknown checkout option');

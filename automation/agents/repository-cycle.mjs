...
const git = (root, args) => execFileSync("git", ["-c", "core.fsmonitor=false", ...args], {
  cwd: root, env: cleanGitEnvironment(), encoding: "utf8", timeout: 10000, stdio: ["ignore", "pipe", "pipe"],
}).trim();
const save = (path, value) => writeFileSync(path, JSON.stringify(value,null,2), {mode:0o600});

export function repositoryAdapterStatus() {
  return {schemaVersion:1,enabled:REPOSITORY_EXECUTION_ENABLED,mode:"manual_prepared_candidate",
    installsHook:false,changesTrust:false,createsWorktree:false,acquiresLease:false,
    schedulesWork:false,externalActions:false,productionValidated:false};
}
...
export function prepareRepositoryCandidate(input) {
  ...
  const m = {...input,schemaVersion:1,scope:"repository",phase:"implementation",trigger:"human_request",parentRunId:null,
    createdAt,expiresAt,
    runtimeFingerprint:runtime.fingerprint,controllerRuntimeFingerprint:controller.fingerprint,
    hookPath:join(sourceRoot,"automation/agents/repository-work.mjs")};
  for(const key of ["controlRoot","worktree","stateHome","evidenceRoot"]) m[key]=realpathSync(m[key]);
  ...
  m.commands=input.commands.map(command=>({...command,executable:process.execPath,
    script:realpathSync(command.script),command:`'${process.execPath}' '${realpathSync(command.script)}'`,
    digest:digest(readFileSync(command.script))}));
  validateRepositoryManifest(m);
  return {root:m.evidenceRoot,manifest:m,hookCommand:`'${process.execPath}' '${m.hookPath}'`,
    requiredHookSource:join(m.controlRoot,".codex/hooks.json"),readyForExecution:false};
}

function roleContext(m, snapshot) {
  const {leaseToken: _private, ...publicManifest}=m;
  void _private;
  return JSON.stringify({manifest:{...publicManifest,leaseToken:"withheld-controller-only"},
    preflight:{observedAt:new Date().toISOString(),baseSha:snapshot.baseSha,runtimeFingerprint:snapshot.runtime.fingerprint,
      activeClaims:snapshot.activeClaims.map(({threadId,worktree,files})=>({threadId,worktree,files})),
      blockers:snapshot.blockers},
    controllerAttestation:{claimAndLeaseVerified:m.phase!=="discovery",phase:m.phase,
...
export async function runRepositoryRole(prepared, role, task, options={}) {
  ...
  assert(prepared.root===m.evidenceRoot && prepared.hookCommand===`'${process.execPath}' '${m.hookPath}'`,"repository_prepared_binding");
...
export function measureRepositoryCandidate(m) {
  ...
  assert(validateDiff({changedPaths,allowedPaths:m.allowedPaths,addedLines:rows.reduce((n,r)=>n+Number(r[0]),0),
    deletedLines:rows.reduce((n,r)=>n+Number(r[1]),0)},loadPolicy()).ok,"repository_diff_rejected");
...
export function readRepositoryActivation(m) {
  ...
  const result=spawnSync(process.execPath,["automation/agents/runtime.mjs","exec","--",process.execPath,
    ...
    cwd:m.worktree,env:cleanGitEnvironment(),encoding:"utf8",timeout:30000,maxBuffer:1024*1024,
  });
...
export async function runRepositoryCandidate(prepared) {
  ...
  const before=readRepositoryActivation(m);assertActivationUnchanged(m.activationBaseline,before);
  const baseline=measureRepositoryCandidate(m);assert(baseline.changedPaths.length===0,"repository_dirty_candidate");
  let outcome;
  try {
    ...
    assertRepositoryOwnership(m);assertActivationUnchanged(before,readRepositoryActivation(m));
    m.phase="review";
    ...
    outcome={status:"local_candidate_reviewed",runId:m.runId,...measured,roles:[builder.threadId,auditor.threadId],
      fullQualityGate:"pending_controller_evidence",remoteQualityGate:"not_run",autonomyActivated:false,externalAction:"none"};
    return outcome;
  } catch(error) {
    outcome={status:"blocked",runId:m.runId,reason:error.code==="ERR_ASSERTION"?error.message:"repository_candidate_failed",autonomyActivated:false};
    throw error;
  } finally {
    // Evidence is not a RunReport and must not be passed off as a complete cycle.
    save(join(m.evidenceRoot,"repository-outcome.json"),{...outcome,hostTerminationConfirmed:prepared.hostTerminationConfirmed===true,
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  ...
  if(command==="status"&&extra.length===0) console.log(JSON.stringify(repositoryAdapterStatus()));
  else { console.error("repository_execution_disabled; use status. No production cycle, trust or settings changed.");process.exitCode=2; }
}
